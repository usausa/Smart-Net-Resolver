...
internal static class ThrowHelper
{
    ...
    public static void ThrowServiceNotRegistered(Type serviceType)
        => throw new InvalidOperationException(
            String.Format(CultureInfo.InvariantCulture, "No service for type '{0}' has been registered.", serviceType));

    [DoesNotReturn]
    public static void ThrowKeyedServiceNotRegistered(Type serviceType, object? serviceKey)
        => throw new InvalidOperationException(
            String.Format(CultureInfo.InvariantCulture, "No service for type '{0}' and key '{1}' has been registered.", serviceType, serviceKey));
